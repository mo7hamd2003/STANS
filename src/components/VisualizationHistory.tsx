                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <Badge
                            variant="outline"
                            className={getAlgorithmColor(run.algorithm)}
                          >
                    ...
                    <div className="ml-7 mt-4 relative">
                      ...
                      <div className="space-y-2 pl-6">
                        {[...Array(Math.min(3, run.steps.length || 3))].map(
                          (_, i) => (
                            <div
                              key={i}
                ...
                {mockRuns.length === 0 && (
                  <div className="text-center py-12 text-muted-foreground">
                    ...
                    <p className="text-sm mt-2">
                      Run an algorithm to see its history here
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {mockRuns
                    ...
                    .map((run) => (
                      <Card
                        key={run.id}
                        className="p-4 border-traffic-medium/20"
                      >
                        <Badge
                          variant="outline"
                          className={`${getAlgorithmColor(run.algorithm)} mb-3`}
                        >
                          {run.algorithm.toUpperCase()}
                        </Badge>
                ...
                <Card className="p-4 bg-muted/50">
                  ...
                  <div className="space-y-2 text-sm">
                    <p>
                      • All algorithms found optimal solution with equal weights
                    </p>
                    <p>
                      • Dijkstra executed fastest (
                      {Math.min(
                        ...mockRuns
                    ...
                    <p>
                      • Kruskal used fewest steps (
                      {Math.min(
                        ...mockRuns
                          .filter((r) => selectedRuns.includes(r.id))
                          .map((r) => r.steps.length || 12),
                      )}{" "}
                      steps)
                    </p>
                  </div>
